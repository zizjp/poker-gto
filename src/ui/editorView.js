...
import { loadRangeSets, saveRangeSets, findRangeSetById, findScenarioById } from "../core/ranges";
...
export function renderEditorView() {
    ...
    const rangeOptions = rangeSets
        ...
        .join("");
    const activeScenario = findScenarioById(activeRangeSet, settings.activeScenarioId);
...
function scenarioTypeLabel(t) {
    const found = SCENARIO_TYPES.find((x) => x.value === t);
    return found ? found.label : t;
}
function renderHandEditor(s) {
    const hands = s.hands;
...
function renderHandGridSection(scenario) {
    const enabled = new Set(scenario.enabledHandCodes ?? []);
    const cellsHtml = [];
    for (let i = 0; i < RANKS.length; i++) {
        for (let j = 0; j < RANKS.length; j++) {
            ...
            let code;
            if (i === j) {
                code = (rowRank + colRank); // AA, KK, ...
            }
            else if (i < j) {
                code = (rowRank + colRank + "s"); // AKs
            }
            else {
                code = (colRank + rowRank + "o"); // AKo
            }
...
export function initEditorViewEvents() {
    const rangeSetSelect = document.getElementById("editorRangeSetSelect");
    const newRangeSetNameInput = document.getElementById("newRangeSetNameInput");
    const createRangeSetBtn = document.getElementById("createRangeSetBtn");
    const deleteRangeSetBtn = document.getElementById("deleteRangeSetBtn");
    const exportRangeSetBtn = document.getElementById("exportRangeSetBtn");
    const importRangeSetBtn = document.getElementById("importRangeSetBtn");
    const importRangeFileInput = document.getElementById("importRangeFileInput");
    const scenarioSelect = document.getElementById("editorScenarioSelect");
    const newScenarioNameInput = document.getElementById("newScenarioNameInput");
    const heroSelect = document.getElementById("newScenarioHeroPosition");
    const villainSelect = document.getElementById("newScenarioVillainPosition");
    const typeSelect = document.getElementById("newScenarioType");
        ...
        importRangeFileInput.addEventListener("change", () => {
            ...
            reader.onload = () => {
                try {
                    ...
                    const rangeSets = loadRangeSets();
                    const existingIndex = rangeSets.findIndex((rs) => rs.meta.id === imported.meta.id);
                    const now = new Date().toISOString();
                    if (!imported.meta.createdAt)
                        imported.meta.createdAt = now;
                    imported.meta.updatedAt = now;
                    if (!imported.meta.version)
                        imported.meta.version = 1;
                    if (!imported.meta.gameType)
                        imported.meta.gameType = "8max_BB_ante";
                    if (existingIndex >= 0) {
                        const overwrite = window.confirm(`同じIDのレンジセット「${rangeSets[existingIndex].meta.name}」が存在します。\n\nOK: 上書き\nキャンセル: 新しいIDで追加`);
                        if (overwrite) {
    ...
    if (createScenarioBtn) {
        createScenarioBtn.addEventListener("click", () => {
            ...
            const hero = (heroSelect?.value || "BTN");
            const villainRaw = villainSelect?.value || "";
            const villain = (villainRaw || undefined);
            const type = (typeSelect?.value || "OPEN");
    ...
    if (deleteScenarioBtn) {
        deleteScenarioBtn.addEventListener("click", () => {
            ...
            if (!rangeSet)
                return;
            if (rangeSet.scenarios.length === 0)
                return;
            const currentScenario = findScenarioById(rangeSet, settings.activeScenarioId) ?? rangeSet.scenarios[0];
            if (!currentScenario)
                return;
...
function initHandEditorEvents(rangeSets, rangeSet, scenario) {
    const rows = document.querySelectorAll(".hand-row");
    rows.forEach((row, index) => {
        const code = row.getAttribute("data-hand");
        if (!code)
            return;
        const header = row.querySelector(".hand-row-header");
        const body = row.querySelector(".hand-row-body");
        const raiseInput = row.querySelector(".hand-raise-input");
        const callInput = row.querySelector(".hand-call-input");
        const foldDisplay = row.querySelector(".hand-fold-display");
        const errorEl = row.querySelector(".hand-error");
        const saveBtn = row.querySelector(".hand-save-button");
        const summaryEl = row.querySelector(".hand-row-summary");
        if (!header || !body || !raiseInput || !callInput || !foldDisplay || !errorEl || !saveBtn || !summaryEl)
            return;
        header.addEventListener("click", () => {
            const isOpen = body.style.display !== "none";
            body.style.display = isOpen ? "none" : "block";
            const toggle = header.querySelector(".hand-row-toggle");
            if (toggle)
        ...
        saveBtn.addEventListener("click", () => {
            ...
            if (nextRow) {
                const nextHeader = nextRow.querySelector(".hand-row-header");
                const nextBody = nextRow.querySelector(".hand-row-body");
                if (nextHeader && nextBody) {
                    nextBody.style.display = "block";
                    const t = nextHeader.querySelector(".hand-row-toggle");
                    if (t)
                        t.textContent = "▲";
                }
            }
...
function initGlobalHandGridEvents() {
    if (handGridGlobalEventsInitialized)
        return;
    ...
    document.addEventListener("click", (ev) => {
        ...
        if (!target)
            return;
        // セルクリック
        const cell = target.closest(".hand-grid-cell");
        if (cell) {
            const code = cell.getAttribute("data-hand");
            if (!code)
                return;
            const { rangeSets, rangeSet, scenario } = getActiveRangeSetAndScenario();
            if (!rangeSets || !rangeSet || !scenario)
                return;
            const enabled = new Set(scenario.enabledHandCodes ?? []);
            if (enabled.has(code)) {
                enabled.delete(code);
        ...
        // 全てON
        const allOnBtn = target.closest("#handGridAllOnBtn");
        if (allOnBtn) {
            const { rangeSets, rangeSet, scenario } = getActiveRangeSetAndScenario();
            if (!rangeSets || !rangeSet || !scenario)
                return;
        ...
        // 全てOFF
        const allOffBtn = target.closest("#handGridAllOffBtn");
        if (allOffBtn) {
            const { rangeSets, rangeSet, scenario } = getActiveRangeSetAndScenario();
            if (!rangeSets || !rangeSet || !scenario)
                return;
        ...
        // プリセットボタン
        const preset25Btn = target.closest("#handGridPreset25");
        if (preset25Btn) {
            applyHandGridPreset(0.25);
            return;
        }
        const preset45Btn = target.closest("#handGridPreset45");
        if (preset45Btn) {
            applyHandGridPreset(0.45);
            return;
        }
        const preset50Btn = target.closest("#handGridPreset50");
        if (preset50Btn) {
            applyHandGridPreset(0.5);
            return;
        }
        const preset100Btn = target.closest("#handGridPreset100");
        if (preset100Btn) {
...
function applyHandGridPreset(ratio) {
    const { rangeSets, rangeSet, scenario } = getActiveRangeSetAndScenario();
    if (!rangeSets || !rangeSet || !scenario)
        return;
    const order = generateHandGridOrder();
...
function persistEnabled(rangeSets, rangeSet, scenario, enabled) {
...
function getAllHandCodes() {
    ...
    for (let i = 0; i < RANKS.length; i++) {
        for (let j = 0; j < RANKS.length; j++) {
            ...
            let code;
            if (i === j) {
                code = (rowRank + colRank);
            }
            else if (i < j) {
                code = (rowRank + colRank + "s");
            }
            else {
                code = (colRank + rowRank + "o");
            }
            result.push(code);
        }
    }
...
function syncCellsFromSet(enabled) {
    const cells = document.querySelectorAll(".hand-grid-cell");
    cells.forEach((cell) => {
        const code = cell.getAttribute("data-hand");
        if (!code)
            return;
 ...
 */
function validateImportedRangeSet(raw) {
    ...
    if (!metaId)
        throw new Error("meta.id がありません。");
    if (!metaName)
        throw new Error("meta.name がありません。");
    const meta = {
        id: metaId,
        name: metaName,
    ...
    const scenarios = scenariosRaw.map((s, idx) => {
        if (!isPlainObject(s)) {
        ...
        const heroPosition = s.heroPosition;
        const villainPosition = (s.villainPosition ?? undefined);
        const scenarioType = s.scenarioType;
        ...
        let enabledHandCodes = [];
        if (Array.isArray(enabledRaw)) {
            enabledHandCodes = enabledRaw.map((h) => String(h));
        }
        else {
            enabledHandCodes = Object.keys(hands);
        }
