...
export function initSettingsViewEvents() {
  ...
  // 正誤判定モード
  const judgeModeInputs = document.querySelectorAll<HTMLInputElement>(
    'input[name="judgeMode"]'
  );
  ...
  // ハプティックON/OFF
  const hapticToggle = document.getElementById("hapticToggle") as HTMLInputElement | null;
  if (hapticToggle) {
  ...
  // データリセット
  const dataResetBtn = document.getElementById("dataResetBtn") as HTMLButtonElement | null;
  if (dataResetBtn) {
    dataResetBtn.addEventListener("click", () => {
      const ok = window.confirm(
        "本当にすべてのデータを削除して初期化しますか？\nレンジ・学習履歴・設定がすべて失われます。"
      );
      if (!ok) return;
